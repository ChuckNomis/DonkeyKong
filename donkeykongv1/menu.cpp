...
void Menu::start(bool isLoad, bool isSilent, bool isSave) {
	...
	if (!isLoad) {
		...
		while (isRunning) {
			if (_kbhit()) { // Check if a key is pressed
				...
				case '1': // Option 1: Start a new game
					...
					currScore = _manualGame.startGame(1, sumOfFiles);  // Start the game
				...
				case '2':
					...
					if (screenNumber != 0) {
						_manualGame.startGame(screenNumber, sumOfFiles);
					}
					_board.setMenu();   // Reset to the menu after the game ends
					_board.print();     // Reprint the menu
					break;
		...
		AutoGame _autoGame(isSilent);
		_autoGame.startGame(1, sumOfFiles);
	}
